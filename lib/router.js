    ...
    return {
        submitRequest : function (req, res, next) {
            ...
            if (_.isUndefined(path)) {
                ...
                res.end('URL should be Ex: /assigningAuthority/identifier/health/clinicalDocuments/historicalData/medicalHistory/serviceTreatmentRecords');
                return;
            }
            ...
            if (_.isUndefined(collection)) {
                ...
                res.end('Path ' + path + 'does not match a collection');
                return;
            }
            ...
            if (_.isUndefined(ssn)) {
                ...
                res.end('SSN could not be fetched from the Subscription message');
                return;
            }

            if (S(req.params.identifier).startsWith('PersonSSNIdentification')) {
                req.params.identifier = ssn;
            }
            ...
            if (_.isUndefined(req.params) || _.isUndefined(req.params.assigningAuthority) || !S(req.params.assigningAuthority).startsWith(config.pix.oids.ssn)) {
                ...
                res.end('Assigning Authority is invalid. Only ' + config.pix.oids.ssn + ' is supported');
                return;
            }
            if (S(req.params.assigningAuthority).startsWith(config.pix.oids.ssn)) {
                // No need to check URI
                if (_.isUndefined(req.params) || !_.isUndefined(req.params.identifier) && !S(req.params.identifier).startsWith(ssn)) {
                    ...
                    res.end('URL Identifier does not match with SSN in the message - URL:' + req.params.identifier + ' Message:' + ssn);
                    return;
                }
                ...
                if (_.isUndefined(dob)) {
                    ...
                    res.end('DoB is missing in the message');
                    return;
                }
                if (_.isUndefined(firstName)) {
                    ...
                    res.end('First Name is missing in the message');
                    return;
                }
                if (_.isUndefined(lastName)) {
                    ...
                    res.end('Last Name is missing in the message');
                    return;
                }
                if (_.isUndefined(gender)) {
                    ...
                    res.end('Gender is missing in the message');
                    return;
                }
                ...
                request(config.pix.url + '/' + req.params.assigningAuthority + "/" + ssn, function (error, response, body) {
                    ...
                    if (!error && response.statusCode == 200) {
                        ...
                        if (_.isUndefined(json)) {
                            logger.error('Service should return atleast a value for ' + config.pix.oids.aun + ' (AUN):', json);
                            res.writeHead(500, 'Service should return atleast a value for ' + config.pix.oids.aun + ' (AUN)');
                            res.end('Service should return atleast a value for ' + config.pix.oids.aun + ' (AUN)');
                            return;
                        }
                        var aun;
                            ...
                            crudOperation.attempt(function (currentAttempt) {
                                ...
                                request(config.ecrud.url + '/'
                                    ...
                                    if (!error && response.statusCode == 200) {
                                        ...
                                        // Already Subscribed
                                        if (json.length > 0) {
                                            logger.trace('Veteran already subscribed:', json);
                                            res.writeHead(200, 'Veteran already subscribed');
                                            res.end('Veteran already subscribed');
                                            return;
                                        } else {
                                            ...
                                            if (_.isUndefined(newClient)) {
                                                req.body['subscription:Subscription']['subscription:CommonData']['vler:Client'] = JSON.parse('{"vler:ClientIdentifier":{"nc:IdentificationID":"' + aun + '", "vler:AssigningAuthority":"' + config.pix.oids.aun + '" }}');
                                            } else {
                                                ...
                                                var jsonToAdd = JSON.parse('{"nc:IdentificationID":"' + aun + '", "vler:AssigningAuthority":"' + config.pix.oids.aun + '" }');
                                                var elementArray = [];
                                        ...
                                        res.end('eCRUD seems to be down! Error:' + error);
                                        return;
                                    }
                                });
                            });
                        ...
                        res.end('SSN to AUN service is returning errors! Error:' + error);
                        return;
                    }
                })
            })
